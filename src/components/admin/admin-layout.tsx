...
const navSections: { label: string; items: NavItem[] }[] = [
  ...
  { label: 'Gestión', items: [
    ...
    { href: '/admin/actualizar-clubes', label: 'Clubes', icon: Building },
  ]},
];
...
export function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="fixed inset-0 flex bg-[var(--bg)]">
      {/* Sidebar */}
      <aside className={`${sidebarW} flex-shrink-0 border-r border-[var(--border)] bg-[var(--bg-secondary)] flex flex-col transition-all duration-300`}>
        {/* Logo */}
        ...
        <div className="flex-shrink-0 bg-[var(--bg-secondary)] border-t border-[var(--border)]">
          ...
          {!collapsed && (
            <div className="flex items-center gap-2 px-3 py-2.5">
              ...
              <div className="flex-1 min-w-0"><p className="text-[11px] font-medium text-[var(--text)] truncate leading-tight">{user.email}</p></div>
