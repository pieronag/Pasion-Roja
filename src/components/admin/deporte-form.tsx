...
export function DeporteForm({ deporte, onClose }: { deporte?: Deporte; onClose?: () => void }) {
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true);
    try {
      const data = { nombre: nombre.trim(), icono: icono.trim() || '⚽', activo: true, orden: deporte?.orden || 0, sistemaPuntos: { victoria: parseInt(victoria) || 3, empate: parseInt(empate) || 1, derrota: 0 }, estadisticasDisponibles: ['goles', 'asistencias', 'tarjetas'] };
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="space-y-1"><Label className="text-xs text-[var(--text-muted)]">Nombre</Label><Input value={nombre} onChange={(e) => setNombre(e.target.value)} placeholder="Ej: Fútbol" /></div>
        <div className="space-y-1"><Label className="text-xs text-[var(--text-muted)]">Icono</Label><Input value={icono} onChange={(e) => setIcono(e.target.value)} placeholder="Ej: ⚽" /></div>
      </div>
      <div className="grid grid-cols-2 gap-3">
      ...
      <div className="flex justify-end gap-2 pt-1">
        {onClose && <Button type="button" variant="ghost" size="sm" onClick={onClose}><X className="h-3.5 w-3.5 mr-1" /> Cancelar</Button>}
        <Button onClick={handleSubmit} loading={saving} disabled={success} size="sm"><Save className="h-3.5 w-3.5 mr-1" /> {deporte ? 'Guardar' : 'Crear'}</Button>
      </div>
    </div>
  );
}
