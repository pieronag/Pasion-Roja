  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-3">
        <div className="relative flex-1 max-w-sm"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-[var(--text-muted)]" /><Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar equipo..." className="pl-9" /></div>
        <div className="w-48"><Select value={filterDeporte} onValueChange={setFilterDeporte}><SelectTrigger><SelectValue placeholder="Todos los deportes" /></SelectTrigger><SelectContent><SelectItem value="">Todos los deportes</SelectItem>{deportes.map((d) => <SelectItem key={d.id} value={d.id}>{d.icono} {d.nombre}</SelectItem>)}</SelectContent></Select></div>
      </div>
                  ...
                  <tr key={e.id} className="border-b border-[var(--border)] hover:bg-[var(--bg-hover)] transition-colors">
                    <td className="p-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div><p className="font-medium text-[var(--text)]">{e.nombre}</p><p className="text-xs text-[var(--text-muted)]">{e.nombreCorto}{e.entrenador ? ` · DT: ${e.entrenador}` : ''}</p></div>
                      </div>
                    </td>
                    <td className="p-3 text-sm text-[var(--text-secondary)]">{deporte?.icono} {deporte?.nombre || '—'}</td>
                    <td className="p-3"><div className="flex items-center gap-1.5 text-sm text-[var(--text-secondary)]"><MapPin className="h-3.5 w-3.5" />{e.ciudad || e.estadio || '—'}</div></td>
                    <td className="p-3 text-right">
                      <div className="flex items-center justify-end gap-1">
                        <Link href={`/admin/equipos/${e.id}/jugadores`}><Button variant="ghost" size="sm" className="h-8"><Users className="h-3.5 w-3.5 mr-1" /> Plantilla</Button></Link>
