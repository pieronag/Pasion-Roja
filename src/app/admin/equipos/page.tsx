...
export default function AdminEquiposPage() {
  ...
  const [allDivisiones, setAllDivisiones] = useState<Division[]>([]); // ALL divisions (for display)
  const [filterDivisiones, setFilterDivisiones] = useState<Division[]>([]); // Filtered for dropdown
  ...
  // Load ALL divisiones (needed for display in table regardless of filter)
  useEffect(() => {
        ...
        <div className="border border-[var(--border)] rounded-[var(--radius)] overflow-hidden">
          <table className="w-full">
            <thead><tr className="bg-[var(--bg-secondary)] border-b border-[var(--border)]">
              ...
              <th className="text-right p-3 text-xs font-semibold text-[var(--text-muted)] uppercase">Jug.</th>
            ...
            <tbody>
              {filtered.map((e) => {
                ...
                const jugCount = 0; // Will be dynamic with a count
                return (
                  <tr key={e.id} className="border-b border-[var(--border)] hover:bg-[var(--bg-hover)] transition-colors">
                    ...
                    <td className="p-3 text-sm text-[var(--text-secondary)]">{division?.nombre || <span className="text-[var(--text-muted)]">—</span>}</td>
                    ...
                    <td className="p-3 text-right text-sm text-[var(--text-secondary)]">{jugCount}</td>
                    <td className="p-3 text-right">
                      <div className="flex justify-end gap-1">
                        ...
                        <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => { setEditing(e); setShowCreate(true); }}><Pencil className="h-3.5 w-3.5" /></Button>
                        <Button variant="ghost" size="icon" className="h-7 w-7 text-red-400" onClick={() => handleDelete(e.id)}><Trash2 className="h-3.5 w-3.5" /></Button>
                      </div>
                    </td>
                  </tr>
                );
