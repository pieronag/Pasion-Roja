...
export default function AdminProgramacionPage() {
  ...
  return (
    <div className="space-y-5">
      <div><h2 className="text-lg font-bold text-[var(--text)]">Programación</h2><p className="text-sm text-[var(--text-secondary)]">Gestiona la programación de radio y TV</p></div>
      ...
      <Card className="overflow-hidden">
        ...
        <CardContent className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1"><Label className="text-[10px] text-[var(--text-muted)]">Título</Label><Input value={titulo} onChange={(e) => setTitulo(e.target.value)} placeholder="Ej: La Mañana Deportiva" /></div>
            <div className="space-y-1"><Label className="text-[10px] text-[var(--text-muted)]">Host</Label><Input value={host} onChange={(e) => setHost(e.target.value)} /></div>
          </div>
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="space-y-1"><Label className="text-[10px] text-[var(--text-muted)]">Categoría</Label><Select value={categoria} onValueChange={setCategoria}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="deportes">Deportes</SelectItem><SelectItem value="noticias">Noticias</SelectItem><SelectItem value="entrevistas">Entrevistas</SelectItem><SelectItem value="musica">Música</SelectItem></SelectContent></Select></div>
            <div className="space-y-1"><Label className="text-[10px] text-[var(--text-muted)]">Día</Label><Select value={dia} onValueChange={setDia}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{['Dom','Lun','Mar','Mié','Jue','Vie','Sáb'].map((d,i) => <SelectItem key={i} value={i.toString()}>{d}</SelectItem>)}</SelectContent></Select></div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1"><Label className="text-[10px] text-[var(--text-muted)]">Inicio</Label><Input type="time" value={horaInicio} onChange={(e) => setHoraInicio(e.target.value)} /></div>
            <div className="space-y-1"><Label className="text-[10px] text-[var(--text-muted)]">Fin</Label><Input type="time" value={horaFin} onChange={(e) => setHoraFin(e.target.value)} /></div>
          </div>
          ...
          <Button onClick={crear} size="sm"><Plus className="h-3.5 w-3.5 mr-1.5" /> Agregar</Button>
        </CardContent>
      </Card>
            ...
            <tbody>
              {programas.map((p) => (
                <tr key={p.id} className="border-b border-[var(--border)] hover:bg-[var(--bg-hover)] transition-colors">
                  ...
                  <td className="p-3 text-right"><ActionsDropdown actions={[{ label: 'Eliminar', icon: <Trash2 className="h-3.5 w-3.5" />, onClick: () => eliminar(p.id), danger: true }]} /></td>
                </tr>
              ))}
            </tbody>
          </table>
