...
import { Trophy, Shield, Users, Swords, Newspaper, HeartHandshake, TrendingUp, Activity, Calendar, Eye, ArrowRight } from 'lucide-react';
...
export default function AdminDashboardPage() {
  ...
  const metrics = [
    { label: 'Deportes', value: deportes.length, icon: Trophy, href: '/admin/deportes', gradient: 'from-orange-500 to-orange-600', change: 12 },
    { label: 'Equipos', value: equipos.length, icon: Shield, href: '/admin/equipos', gradient: 'from-blue-500 to-blue-600', change: 5 },
    { label: 'Jugadores', value: jugadores.length, icon: Users, href: '/admin/jugadores', gradient: 'from-green-500 to-green-600', change: 8 },
    { label: 'Partidos', value: partidos.length, icon: Swords, href: '/admin/partidos', gradient: 'from-red-500 to-red-600', change: 15 },
    { label: 'Noticias', value: noticias.length, icon: Newspaper, href: '/admin/noticias', gradient: 'from-purple-500 to-purple-600', change: 3 },
    { label: 'Sponsors', value: sponsors.length, icon: HeartHandshake, href: '/admin/sponsors', gradient: 'from-yellow-500 to-yellow-600', change: 0 },
  ];
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4">
        {metrics.map((m) => (
          <MetricCard key={m.label} {...m} changeLabel="este mes" />
        ))}
      </div>
      ...
      {enVivo > 0 && (
        <div className="flex items-center gap-3 p-4 rounded-[var(--radius)] bg-red-500/10 border border-red-500/20">
          ...
          <p className="text-sm font-medium text-[var(--text)]">{enVivo} partido(s) en vivo ahora — <Link href="/admin/partidos" className="text-[var(--accent)] hover:underline">Ir a partidos →</Link></p>
        </div>
      )}
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-[var(--border)] bg-[var(--bg-secondary)]">
                    <th className="text-left p-3 text-xs font-semibold text-[var(--text-muted)] uppercase">Jornada</th>
                ...
                <tbody>
                  {partidos.slice(0, 5).map((p) => (
                    <tr key={p.id} className="border-b border-[var(--border)] hover:bg-[var(--bg-hover)] transition-colors">
                      ...
                      <td className="p-3 text-center"><span className="font-bold font-display text-base text-[var(--text)]">{p.marcadorLocal}</span><span className="text-[var(--text-muted)] mx-1">-</span><span className="font-bold font-display text-base text-[var(--text)]">{p.marcadorVisita}</span></td>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <div className="flex items-center justify-between p-3 rounded-[var(--radius-sm)] bg-emerald-500/5 border border-emerald-500/10">
              ...
              <span className="text-lg font-bold text-emerald-500">{finalizados}</span>
            </div>
            ...
            <div className="flex items-center justify-between p-3 rounded-[var(--radius-sm)] bg-red-500/5 border border-red-500/10">
              <div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-red-500 animate-ping-slow" /><span className="text-sm text-[var(--text)]">En Vivo</span></div>
